...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        previous_state_level = self.s_levels[level] 
        
        for action in possible_actions:
            node_a = PgNode_a(action)
            
            if node_a.prenodes.issubset(previous_state_level):
                a_nodes.append(node_a)
                
                for node_s in previous_state_level:
    ...
    def add_literal_level(self, level):
        ...
        s_nodes = set()
        for node_a in previous_action_level:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return bool(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem) |
    ...
    def h_levelsum(self) -> int:
        ...
        state_levels = self.s_levels
        
        for goal in goals:
            node = PgNode_s(goal, True)
            state_levels_list = enumerate(state_levels)
            for level, s_nodes in state_levels_list:
                if node in s_nodes:
                    level_sum += level
                    break
        return level_sum